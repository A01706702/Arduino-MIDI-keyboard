# Arduino-MIDI-keyboard

Turn old toy keyboard (CASIO ToneBank MA-120) into a fully functional MIDI synth for music production.

## Circuit Diagram Version 1
![state machine](images/midi_circuit2.png)

## Circuit Diagram Version 2
![state machine](images/MIDI_KEYBOARD_V2.png)

## Final result
![state machine](images/final_result.jpg)
![state machine](images/finalfinal.jpg)


https://github.com/A01706702/Arduino-MIDI-keyboard/assets/65085262/0da4adef-ebb8-43b7-9c0b-2581038c9dfc

## References:
- https://github.com/hanomaly/old-keyboard-to-midi-controller/tree/master
- https://github.com/evankale/ArduinoMidiPiano
- https://youtu.be/UBMLq3aa4Ok?si=S3osB-YkyYTNgnNi
- https://midi.org/expanded-midi-1-0-messages-list
- https://midi.org/midi-1-0-control-change-messages
